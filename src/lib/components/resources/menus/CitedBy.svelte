<script lang="ts">
    ...
    interface CitedByResponse {
        citedByContent: AssociatedResource[];
    }
    ...
    async function lazyLoadCitedBy() {
        if (contentLoaded) return;

        isLoading = true;

        try {
            const response = await getFromApi<CitedByResponse>(`/resources/content/${currentResourceId}/cited-by`);
            relatedContent = response.citedByContent;
        } catch (error) {
            log.exception(error);
        } finally {
            isLoading = false;
...
<div class="dropdown ms-1" data-app-insights-event-name="cited-by-menu-click" onclick={lazyLoadCitedBy}>
