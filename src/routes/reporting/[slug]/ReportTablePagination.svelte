...
<div class="flex h-full w-auto items-center justify-between space-x-2">
    <button class="btn btn-link disabled:bg-white" onclick={handleLeftClick} disabled={paginationStart === 0}>
        <Icon data={chevronLeft} />
    </button>

    <span>{paginationStart + 1} - {paginationEnd} of {totalCount}</span>

    <button class="btn btn-link disabled:bg-white" onclick={handleRightClick} disabled={paginationEnd === totalCount}>
