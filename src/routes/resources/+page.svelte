<script lang="ts">
    ...
    async function applyFilters() {
        if (canApplyFilters) {
            const { start, end } = parseStartAndEndFromSingleOrRangeString(chapterRange, 1, calculateMaxChapter());
    ...
    function calculateMaxChapter() {
        return data.bibles?.[0]?.books.find((b) => b.bookCode === bookCode)?.chapterCount ?? 0;
    }
</script>
    ...
    <div class="flex flex-row space-x-2 overflow-x-scroll px-4 py-2">
        ...
        <Select
            ...
            options={[
                ...
                ...(data.bibles?.[0]?.books || []).map((b) => ({ value: b.bookCode, label: b.displayName })),
            ]}
